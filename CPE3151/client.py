...
def receiveMsg(conn, private_key):
    ...
    try:
        while run:
            ...
            if not verify_signature(server_public, ciphertext, signature):
...
if __name__ == '__main__':
    ...
    server_pub_bytes = recv_with_length(s)
    server_public = deserialize_public(server_pub_bytes)

    client_pub_bytes = serialize_public(public_key)
    send_with_length(s, client_pub_bytes)

    rcv = Thread(target=receiveMsg, args=(s, private_key))
    snd = Thread(target=sendMessage, args=(s, server_public))
    rcv.start()
    snd.start()
